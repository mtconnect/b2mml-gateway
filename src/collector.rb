...
class Collector
  ...
  def stream
    ...
    while $running
      begin
        logger.info "Connecting to agent: #{@nane} from #{@url}"
  ...
  def parse_sample(xml)
    @@mutex.synchronize do
      ...
      @program = 'unknown' unless @program
      if @next.nil?
        logger.info "#{@name} (#{@url}) Recovering from position #{nxt}"
        @next = nxt          
        write_recovery
      elsif nxt > @next
        @next = nxt
    ...
    logger.error "#{@name}: A stream error occured while parsing: #{e}"
    logger.error e.to_s
    logger.error e.backtrace.join("\n")
    false
    
  rescue StandardError => e
    logger.error "#{@name}: An error occured while parsing: #{e}"
    logger.error e.to_s
    logger.error e.backtrace.join("\n")
    false
